...
const Icon = ({ symbol, className = '', size = 16 }) => (
  <span
    className={`inline-flex items-center justify-center select-none ${className}`}
    style={{ width: size, height: size, fontSize: Math.max(12, Math.round(size * 0.82)) }}
    aria-hidden="true"
  >
    {symbol}
  </span>
...
const Target = (props) => <Icon symbol="T" {...props} />;
const ChevronRight = (props) => <Icon symbol=">" {...props} />;
const CheckCircle2 = (props) => <Icon symbol="V" {...props} />;
const AlertCircle = (props) => <Icon symbol="!" {...props} />;
const Briefcase = (props) => <Icon symbol="B" {...props} />;
const TrendingUp = (props) => <Icon symbol="^" {...props} />;
const Zap = (props) => <Icon symbol="Z" {...props} />;
const ClipboardCheck = (props) => <Icon symbol="C" {...props} />;
const Rocket = (props) => <Icon symbol="R" {...props} />;
const Edit3 = (props) => <Icon symbol="E" {...props} />;
const RefreshCw = (props) => <Icon symbol="O" {...props} />;
const Save = (props) => <Icon symbol="S" {...props} />;
const ListChecks = (props) => <Icon symbol="L" {...props} />;
const X = (props) => <Icon symbol="X" {...props} />;
const ShieldAlert = (props) => <Icon symbol="A" {...props} />;
const UserCheck = (props) => <Icon symbol="U" {...props} />;
...
const normalizeInsight = (rawText) => {
  ...
  return blocks.length ? blocks : [{ type: 'paragraph', content: safeText }];
};
...
const App = () => {
  ...
  const callGemini = async (prompt, systemPrompt, responseSchema = null) => {
    let retries = 0;
    const maxRetries = 5;
  ...
  const generateGoals = async () => {
    if (!runtimeApiKey.trim()) {
      setError('Ingresa tu API key de Gemini para generar recomendaciones personalizadas.');
      return;
    }
...
4. Usa un tono profesional y enfocado en el puesto actual.`;
    ...
    try {
      ...
      if (err.message === 'API_KEY_MISSING') {
        setError('Falta API key de Gemini. Agregala y vuelve a intentar.');
      } else {
        setError('Error al conectar con Gemini. Revisa tu API key o tu conexion e intenta de nuevo.');
      }
    } finally {
      setLoading(false);
    }
  ...
  const generateActionPlan = async (index) => {
    if (!runtimeApiKey.trim()) {
      setError('Ingresa tu API key de Gemini para generar planes de accion.');
      return;
    }
  ...
  const analyzeRisks = async (index) => {
    if (!runtimeApiKey.trim()) {
      setError('Ingresa tu API key de Gemini para analizar riesgos.');
      return;
    }
          ...
          <div className="flex flex-col md:flex-row gap-3 md:items-end">
            <div className="flex-1">
              ...
              <input
                ...
                placeholder="Pega tu API key (AI Studio) para activar recomendaciones IA"
